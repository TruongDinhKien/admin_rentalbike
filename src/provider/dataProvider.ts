...
export const filterFromParams = (params: any) => {
  ...
  if (filter.where.q) {
    filter.where.textSearch = {
      ilike: `%${filter.where.q}%`,
    }
...
const provider = (apiUrl: string, httpClient = fetchJson) => {
      ...
      case GET_LIST: {
        ...
        if (params) {
          Object.keys(params).forEach(key => {
            if (!specialParams.has(key) && params[key] !== undefined) query[key] = params[key]
          })
        }
        urls = [
        ...
        switch (options.method) {
          case 'GET': {
            ...
            if (params.filter) {
              ...
              url += connectCharactor + `${stringify({ filter: JSON.stringify(filter), ...rootQuery })}`
            }
            break
          }
          case 'POST': {
  ...
  const convertWithCountHTTPResponse = (response: any, type: string, resource: string, params: any, count = 0) => {
    switch (type) {
      case GET_LIST:
      case GET_MANY_REFERENCE:
        return {
   ...
   */
  const convertHTTPResponse = (response: any, type: string, resource: string, params: any, count = 0) => {
    const { json } = response
    switch (type) {
      case CREATE:
  ...
  const ret = (type: string, resource: string, params: any) => {
    ...
    const { urls, url, options } = convertDataRequestToHTTP(type, resource, params)
    ...
    if (!urls) {
      ...
      return Promise.all(
        urls.map(url =>
          httpClient(url, options).then(response => convertHTTPResponse(response, type, resource, params)),
        ),
      ).then(responses => {
